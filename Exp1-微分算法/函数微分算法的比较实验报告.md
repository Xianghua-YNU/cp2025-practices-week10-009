...
中心差分法的误差：在光滑区域和远离边界的地方，其误差较小，并出现了一定的规律性。当接近区间的边界时，误差会显著增大。这是因为中心差分法在离边界较近的点，使用的两个点（x-h）和（x+h）分别位于不同区域，计算时可能会放大边界条件的影响。

Richardson外推法的误差：相较于中心差分法，其误差更小，尤其是在靠近中心点时。这是因为Richardson外推法通过结合不同步长的结果，减少了由于步长选择不当引入的误差。
随着外推阶数的增加，Richardson外推法的误差明显减小。

#### 4.2.1 中心差分法误差分析
（插入中心差分法误差随步长变化的log-log图及分析）
在较大的步长下，计算得到的导数误差通常较大。这是因为当步长过大时，数值微分没有足够的精度来捕捉函数的局部变化，从而导致更大的误差。
随着步长的减小，误差应该逐渐减小，符合理论预测。然而，当步长过小（例如接近数值精度限制时）时，误差可能会反弹，这通常是由于计算中的舍入误差和数值不稳定性引起的。

#### 4.2.2 Richardson外推法误差分析
（插入不同阶数Richardson外推法误差随步长变化的log-log图及分析）
随着阶数的增加，相同步长下的误差会逐步减小，表现出更好的精度。

![下载](https://github.com/user-attachments/assets/9067914b-5813-41b6-9a7a-50d12083def5)
## 五、实验讨论
### 5.1 两种方法的优缺点分析
1. 中心差分法
   - 优点：简单易实现，计算效率高
   - 缺点：存在边界效应，对步长敏感，不适用于不光滑函数
2. Richardson外推法
   - 优点：高精度，可以通过增加外推阶数，不断提高计算精度
   - 缺点：实现复杂，计算量大，对步长选择敏感
